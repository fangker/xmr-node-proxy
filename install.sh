...
if which yum >/dev/null; then
  sudo yum -y update
  sudo yum -y upgrade
  sudo yum -y install git curl make gcc-c++ python boost-devel boost-system-devel boost-date-time-devel libsodium-devel
else
  sudo apt-get update
...
cd ~
git clone https://github.com/fangker/xmr-node-proxy
